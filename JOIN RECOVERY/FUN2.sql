-- FUNCTION: omp_owner.nifi_md_tam_legacy_hdr_raw_upsert_recovery_fn(integer)

-- DROP FUNCTION omp_owner.nifi_md_tam_legacy_hdr_raw_upsert_recovery_fn(integer);

CREATE OR REPLACE FUNCTION omp_owner.nifi_md_tam_legacy_hdr_raw_upsert_recovery_fn(
...
DECLARE
    ...
    v_tabla_reg  	nifi_md_tam_legacy_hdr_raw%ROWTYPE;
...
BEGIN
	...
	select id, payload from nifi_md_tam_legacy_hdr_raw_header where id = p_raw_header_id into v_id, v_xml;
...
ALTER FUNCTION omp_owner.nifi_md_tam_legacy_hdr_raw_upsert_recovery_fn(integer)
    OWNER TO omp_owner;

GRANT EXECUTE ON FUNCTION omp_owner.nifi_md_tam_legacy_hdr_raw_upsert_recovery_fn(integer) TO PUBLIC;

GRANT EXECUTE ON FUNCTION omp_owner.nifi_md_tam_legacy_hdr_raw_upsert_recovery_fn(integer) TO omp_owner;

GRANT EXECUTE ON FUNCTION omp_owner.nifi_md_tam_legacy_hdr_raw_upsert_recovery_fn(integer) TO omp_rw_role;
